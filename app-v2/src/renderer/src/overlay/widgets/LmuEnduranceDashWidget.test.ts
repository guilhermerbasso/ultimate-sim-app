...
function render(snapshot: TelemetrySnapshot | null, cfg: OverlayWidgetConfig = config): string {
  return renderToStaticMarkup(createElement(LmuEnduranceDashWidget, { snapshot, config: cfg }))
}
...
describe('LmuEnduranceDashWidget — overflow-safe SVG scene', () => {
  ...
  it('shows core telemetry values via labelled cells', () => {
    ...
    for (const label of ['TYRE', 'BRAKE', 'RPM', 'LAST', 'FUEL']) {
