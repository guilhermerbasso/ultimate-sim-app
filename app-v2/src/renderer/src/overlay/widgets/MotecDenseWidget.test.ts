...
function render(snapshot: TelemetrySnapshot | null, cfg: OverlayWidgetConfig = config): string {
  return renderToStaticMarkup(createElement(MotecDenseWidget, { snapshot, config: cfg }))
}
...
describe('MotecDenseWidget — overflow-safe SVG scene', () => {
  ...
  it('shows core telemetry values via labelled cells', () => {
    ...
    for (const label of ['WATER', 'OIL', 'DELTA', 'MAP', 'BB']) {
