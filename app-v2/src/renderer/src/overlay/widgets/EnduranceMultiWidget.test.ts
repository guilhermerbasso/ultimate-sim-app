...
function render(snapshot: TelemetrySnapshot | null, cfg: OverlayWidgetConfig = config): string {
  return renderToStaticMarkup(createElement(EnduranceMultiWidget, { snapshot, config: cfg }))
}
...
describe('EnduranceMultiWidget — overflow-safe SVG scene', () => {
  ...
  it('shows core telemetry values via labelled cells', () => {
    ...
    expect(m).toContain('2.74')
    expect(m).toContain('88')
    for (const label of ['ENDURANCE', 'STINT', 'FUEL', 'WATER', 'TYRE', 'PRESS']) {
