...
function render(snapshot: TelemetrySnapshot | null): string {
  return renderToStaticMarkup(createElement(GridStackDashWidget, { snapshot, config }))
}
...
describe('GridStackDashWidget — overflow-safe SVG scene', () => {
  ...
  it('shows core telemetry values via labelled cells', () => {
    ...
    for (const label of ['BEHIND', 'AHEAD', 'FUEL', 'DELTA', 'SOF']) {
