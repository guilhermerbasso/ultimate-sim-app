...
function render(snapshot: TelemetrySnapshot | null, cfg: OverlayWidgetConfig = config): string {
  return renderToStaticMarkup(createElement(GT3ClusterWidget, { snapshot, config: cfg }))
}
...
describe('GT3ClusterWidget — overflow-safe SVG scene', () => {
  ...
  it('shows core telemetry values via labelled cells', () => {
    ...
    for (const label of ['FUEL', 'LAPS', 'SPEED', 'DELTA', 'MODE']) {
