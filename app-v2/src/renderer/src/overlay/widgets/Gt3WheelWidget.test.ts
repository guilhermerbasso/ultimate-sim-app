...
function render(snapshot: TelemetrySnapshot | null, cfg: OverlayWidgetConfig = config): string {
  return renderToStaticMarkup(createElement(Gt3WheelWidget, { snapshot, config: cfg }))
}
...
describe('Gt3WheelWidget — overflow-safe SVG scene', () => {
  ...
  it('shows core telemetry values via labelled cells', () => {
    const m = render(POP)
    expect(m).toContain('56.5')
    expect(m).toContain('PIT')
    for (const label of ['TC', 'ABS', 'MAP', 'BB']) {
