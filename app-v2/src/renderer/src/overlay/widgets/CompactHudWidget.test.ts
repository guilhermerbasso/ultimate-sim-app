...
function render(snapshot: TelemetrySnapshot | null, cfg: OverlayWidgetConfig = config): string {
  return renderToStaticMarkup(createElement(CompactHudWidget, { snapshot, config: cfg }))
}
...
describe('CompactHudWidget — overflow-safe SVG scene', () => {
  ...
  it('shows core telemetry values via labelled cells', () => {
    ...
    for (const label of ['SPEED', 'POS', 'TRACK', 'SOF', 'TIME']) {
  ...
  it('exposes the resolved design family and re-skins across presets', () => {
    expect(render(POP)).toContain('data-family="')
    expect(render(POP)).toContain('data-skin="hud"')
    const analog = render(POP, { ...config, stylePreset: 'analog' } as OverlayWidgetConfig)
    const neon = render(POP, { ...config, stylePreset: 'neon' } as OverlayWidgetConfig)
    expect(analog).toContain('data-family="analog"')
    expect(neon).toContain('data-family="neon"')
    expect(analog).not.toBe(neon)
  })
