...
function render(snapshot: TelemetrySnapshot | null, cfg: OverlayWidgetConfig = config): string {
  return renderToStaticMarkup(createElement(RingDashWidget, { snapshot, config: cfg }))
}
...
describe('RingDashWidget — overflow-safe SVG scene', () => {
  ...
  it('shows core telemetry values via labelled cells', () => {
    ...
    for (const label of ['FUEL', 'WATER', 'LAST', 'BEST', 'TC']) {
