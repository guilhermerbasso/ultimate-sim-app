...
export function BrakeTempCornersWidget({ snapshot, config }: WidgetProps): ReactElement {
  ...
  return (
    ...
    >
      ...
      <FitText x={W - pad - 16} y={cy} boxW={28} boxH={headerH * 0.9} text={numberOrDash(hottest, 0)} anchor="end" fontFamily={segment.numeric} fill={brakeColor(hottest, skin)} minFontPx={11} maxFontPx={16} />
      <FitText x={W - pad - 2} y={cy} boxW={14} boxH={headerH * 0.6} text="°C" anchor="end" fontFamily={typography.label} fill={palette.textDim} weight={600} minFontPx={9} maxFontPx={11} />
      ...
      {CORNERS.map(([key, label], i) => {
        ...
        return <Cell key={String(key)} rect={rect} label={label} value={numberOrDash(v, 0)} color={brakeColor(v, skin)} skin={skin} />
      })}

      <FitText x={W / 2} y={H - pad - footerH / 2 + 1} boxW={W - pad * 2} boxH={footerH} text={`Optimal ${BRAKE_COLD}–${BRAKE_OPT_HI} °C`} anchor="middle" fontFamily={typography.label} fill={palette.textDim} weight={600} letterSpacing={0.5} minFontPx={9} maxFontPx={12} />
    </svg>
  )
}
