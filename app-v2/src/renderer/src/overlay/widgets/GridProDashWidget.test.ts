...
function render(snapshot: TelemetrySnapshot | null, cfg: OverlayWidgetConfig = config): string {
  return renderToStaticMarkup(createElement(GridProDashWidget, { snapshot, config: cfg }))
}
...
describe('GridProDashWidget — overflow-safe SVG scene', () => {
  ...
  it('shows core telemetry values via labelled cells', () => {
    ...
    for (const label of ['WATER', 'OIL', 'FUEL', 'POS', 'LAST']) {
