...
function DashTyres({ snapshot }: { snapshot: HifiWidgetProps['snapshot'] }): ReactElement {
  const lf = tyrePsi(snapshot, 'lf')
  const rf = tyrePsi(snapshot, 'rf')
  const lr = tyrePsi(snapshot, 'lr')
  const rr = tyrePsi(snapshot, 'rr')
  const value = (label: string, x: number, y: number, v: number | undefined) => (
    <g>
      <text x={x - 96} y={y - 30} fill={CORVETTE_YELLOW} fontFamily={FONT_LABEL} fontWeight={900} fontSize={25} {...legibleStroke(25)}>{label}</text>
      <text x={x} y={y} textAnchor="end" fill={v == null ? C.dim : WHITE} fontFamily={FONT_NUM} fontWeight={900} fontSize={48} {...legibleStroke(48)}>{fixed(v, 1)}</text>
    </g>
  )
  return (
    <g>
      ...
      {value('RL', 404, 521, lr)}
      {value('RR', 704, 521, rr)}
      <line x1={410} y1={464} x2={474} y2={464} stroke={CORVETTE_YELLOW} strokeWidth={2} />
      <line x1={550} y1={464} x2={614} y2={464} stroke={CORVETTE_YELLOW} strokeWidth={2} />
      <CarDiagram x={482} y={426} scale={0.64} />
      <text x={512} y={568} textAnchor="middle" fill={CORVETTE_YELLOW} fontFamily={FONT_LABEL} fontWeight={900} fontSize={30} letterSpacing={2} {...legibleStroke(30)}>psi</text>
    </g>
  )
}

function CorvetteDash({ snapshot, width, height }: HifiWidgetProps): ReactElement {
  ...
  return (
    <CleanTile width={width ?? DASH_W} height={height ?? DASH_H}>
      ...
      <text x={512} y={360} textAnchor="middle" fill={gear == null ? C.dim : WHITE} fontFamily={FONT_BIG} fontWeight={900} fontSize={178} {...legibleStroke(178)}>{gearLabel(gear)}</text>
      <AngledPanel d="M36 376 H286 L302 392 V548 L286 566 H42 L28 552 V390 Z">
        <text x={58} y={428} fill={CORVETTE_YELLOW} fontFamily={FONT_LABEL} fontWeight={900} fontSize={32} letterSpacing={2} {...legibleStroke(32)}>SPD</text>
        <text x={264} y={482} textAnchor="end" fill={speed == null ? C.dim : WHITE} fontFamily={FONT_BIG} fontWeight={900} fontSize={68} {...legibleStroke(68)}>{fixed(speed)}</text>
        <line x1={40} y1={502} x2={286} y2={502} stroke={CORVETTE_YELLOW} strokeWidth={2} />
        <text x={58} y={536} fill={CORVETTE_YELLOW} fontFamily={FONT_LABEL} fontWeight={900} fontSize={30} letterSpacing={2} {...legibleStroke(30)}>FUEL</text>
        <text x={250} y={554} textAnchor="end" fill={fuel == null ? C.dim : WHITE} fontFamily={FONT_BIG} fontWeight={900} fontSize={52} {...legibleStroke(52)}>{fixed(fuel, 0)}</text>
        <text x={264} y={554} fill={CORVETTE_YELLOW} fontFamily={FONT_LABEL} fontWeight={900} fontSize={28} {...legibleStroke(28)}>L</text>
      </AngledPanel>
      <AngledPanel d="M302 390 H722 L734 404 V548 L710 570 H316 L290 548 V404 Z">
        <DashTyres snapshot={snapshot} />
      </AngledPanel>
      <AngledPanel d="M734 392 L750 376 H988 L998 390 V552 L984 566 H750 L734 548 Z">
        <text x={770} y={432} fill={CORVETTE_YELLOW} fontFamily={FONT_LABEL} fontWeight={900} fontSize={34} letterSpacing={2} {...legibleStroke(34)}>LAP</text>
        <text x={956} y={492} textAnchor="end" fill={lap.startsWith('--') ? C.dim : WHITE} fontFamily={FONT_BIG} fontWeight={900} fontSize={64} {...legibleStroke(64)}>{lap}</text>
        <line x1={746} y1={502} x2={988} y2={502} stroke={CORVETTE_YELLOW} strokeWidth={2} />
        <text x={958} y={556} textAnchor="end" fill={delta == null ? C.dim : condColor(delta, { positiveIsGood: false, deadzone: 0.01, good: CORVETTE_YELLOW, bad: RED, neutral: WHITE })} fontFamily={FONT_BIG} fontWeight={900} fontSize={62} {...legibleStroke(62)}>{signed(delta, 2)}</text>
      </AngledPanel>
    </CleanTile>
  )
}
...
function SpeedWidget({ snapshot, width, height }: HifiWidgetProps): ReactElement {
  ...
  const h = height ?? 165
  const speed = num(snapshot?.speedKmh)
  return <CleanTile width={w} height={h}><text x={w / 2} y={h * 0.62} textAnchor="middle" fill={speed == null ? C.dim : WHITE} fontFamily={FONT_BIG} fontWeight={900} fontSize={h * 0.52} {...legibleStroke(h * 0.52)}>{fixed(speed)}</text><text x={w / 2} y={h * 0.85} textAnchor="middle" fill={CORVETTE_YELLOW} fontFamily={FONT_LABEL} fontWeight={900} fontSize={h * 0.16} {...legibleStroke(h * 0.16)}>km/h</text></CleanTile>
}
...
function TyrePressWidget({ snapshot, width, height }: HifiWidgetProps): ReactElement {
  ...
  const data: [string, number | undefined, number, number, 'start' | 'end'][] = [
    ['FL', tyrePsi(snapshot, 'lf'), w * 0.3, h * 0.34, 'end'],
    ['FR', tyrePsi(snapshot, 'rf'), w * 0.7, h * 0.34, 'start'],
    ['RL', tyrePsi(snapshot, 'lr'), w * 0.3, h * 0.73, 'end'],
    ['RR', tyrePsi(snapshot, 'rr'), w * 0.7, h * 0.73, 'start']
  ]
  return (
    <CleanTile width={w} height={h}>
      ...
      {data.map(([label, value, x, y, anchor]) => (
        <g key={label}>
          ...
          <text x={x} y={y} textAnchor={anchor} fill={value == null ? C.dim : WHITE} fontFamily={FONT_NUM} fontWeight={900} fontSize={42} {...legibleStroke(42)}>{fixed(value, 1)}</text>
        </g>
      ))}
      <text x={w / 2} y={h - 8} textAnchor="middle" fill={CORVETTE_YELLOW} fontFamily={FONT_LABEL} fontWeight={900} fontSize={26} letterSpacing={2} {...legibleStroke(26)}>psi</text>
    </CleanTile>
  )
}
...
function WaterWidget({ snapshot, width, height }: HifiWidgetProps): ReactElement {
  ...
  return <SingleMetric width={width ?? 280} height={height ?? 140} label="WATER" value={fixed(v)} unit="C" color={tempColor(v, 80, 105)} />
}
...
export const cvSpeed: HifiWidgetModule = { id: 'cvSpeed', title: 'Chevrolet Corvette Z06 GT3.R speed', description: 'Clean Corvette Z06 GT3.R speed readout in km/h.', category: 'cars', tags: [...TAGS, 'speed', 'clean'], requires: ['speedKmh'], defaultSize: { w: 330, h: 165 }, render: (props) => <SpeedWidget {...props} /> }
...
export const cvFuel: HifiWidgetModule = { id: 'cvFuel', title: 'Chevrolet Corvette Z06 GT3.R fuel', description: 'Clean Corvette Z06 GT3.R fuel liters readout.', category: 'cars', tags: [...TAGS, 'fuel', 'clean'], requires: ['fuelLiters'], defaultSize: { w: 250, h: 140 }, render: (props) => <FuelWidget {...props} /> }
export const cvTyrePress: HifiWidgetModule = { id: 'cvTyrePress', title: 'Chevrolet Corvette Z06 GT3.R tire pressures', description: 'Clean Corvette Z06 GT3.R four-corner tire pressure grid in psi.', category: 'cars', tags: [...TAGS, 'tyre-pressure', 'tire-pressure', 'psi', 'clean'], requires: ['tyres', 'tireColdPressuresKpa'], defaultSize: { w: 390, h: 210 }, render: (props) => <TyrePressWidget {...props} /> }
