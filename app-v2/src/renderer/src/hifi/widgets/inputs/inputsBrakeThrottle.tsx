...
export function InputsBrakeThrottleWidget({ snapshot, width, height }: HifiWidgetProps): ReactElement {
  ...
  return (
    <WidgetSvg w={w} h={h} label="Brake / Throttle" accent={C.cyan}>
      <g>
        <TraceBar x={36} y={66} w={w - 72} label="Throttle" color={C.green} value={snapshot?.throttle} />
        <TraceBar x={36} y={104} w={w - 72} label="Brake" color={C.red} value={snapshot?.brake} />
        <rect x={graphX} y={graphY} width={graphW} height={52} rx={9} fill="#07090b" stroke="rgba(255,255,255,0.10)" />
        {[0.25, 0.5, 0.75].map((f) => (
        ...
        <text x={graphX} y={h - 17} fill={C.dim} fontFamily={FONT_LABEL} fontSize={12} fontWeight={700} letterSpacing={1.6}>
          OVERLAID INPUT CHANNELS
        </text>
      </g>
    </WidgetSvg>
  )
