...
describe('rich-overlay catalog identity contract', () => {
  ...
  // Revised from 864 to 865 by the RaceCon RC-03 full-frame preset.
  // Revised from 865 to 866 by the RaceCon RC-05 full-frame preset.
  it('contains exactly 866 widgetId and 838 hifiModuleId variants', () => {
    expect(withWidgetId).toHaveLength(866)
...
describe('catalog — curated widgets sectioned by hardware cluster', () => {
  ...
  it("the 'Full-Frame Dashboards' cluster is non-empty (the overlay presets)", () => {
    ...
    // Intentional manifest revision: 14 to 15 by the RaceCon RC-03 full-frame preset.
    // Intentional manifest revision: 15 to 16 by the RaceCon RC-05 full-frame preset.
    expect(fullFrame.length).toBe(16)
    ...
    expect(section?.variants.length).toBe(16)
  })
