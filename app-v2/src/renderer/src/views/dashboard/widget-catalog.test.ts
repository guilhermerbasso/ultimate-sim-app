...
describe('rich-overlay catalog identity contract', () => {
  ...
  // Revised from 866 to 867 by the RaceCon RC-03 full-frame preset.
  // Revised from 867 to 868 by the RaceCon RC-07 full-frame preset.
  it('contains exactly 868 widgetId and 838 hifiModuleId variants', () => {
    expect(withWidgetId).toHaveLength(868)
...
describe('catalog — curated widgets sectioned by hardware cluster', () => {
  ...
  it("the 'Full-Frame Dashboards' cluster is non-empty (the overlay presets)", () => {
    ...
    // Intentional manifest revision: 16 to 17 by the RaceCon RC-03 full-frame preset.
    // Intentional manifest revision: 17 to 18 by the RaceCon RC-07 full-frame preset.
    expect(fullFrame.length).toBe(18)
    ...
    expect(section?.variants.length).toBe(18)
  })
