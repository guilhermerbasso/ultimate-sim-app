...
describe('rich-overlay catalog identity contract', () => {
  ...
  // Revised from 863 to 864 by the RaceCon RC-03 full-frame preset.
  // Revised from 864 to 865 by the RaceCon RC-04 full-frame preset.
  it('contains exactly 865 widgetId and 838 hifiModuleId variants', () => {
    expect(withWidgetId).toHaveLength(865)
...
describe('catalog — curated widgets sectioned by hardware cluster', () => {
  ...
  it("the 'Full-Frame Dashboards' cluster is non-empty (the overlay presets)", () => {
    ...
    // Intentional manifest revision: 14 to 15 by the RaceCon RC-04 full-frame preset.
    expect(fullFrame.length).toBe(15)
    ...
    expect(section?.variants.length).toBe(15)
  })
