...
export default function SetupsView({ showToast, language }: AppViewProps): ReactElement {
  ...
  async function addFolderSource(): Promise<void> {
    try {
      ...
      showToast(tt(language, 'setups.localSourceAdded'), 'success')
    } catch (error) {
      showToast(getErrorMessage(error), 'error')
    }
  }
  ...
  async function addUrlSource(): Promise<void> {
    ...
    if (!isHttpsUrl(url)) {
      showToast(tt(language, 'setups.validHttpsUrl'), 'error')
      return
    }
    ...
    try {
      ...
      setUrlDraft('')
      setSelectedSourceId(source.id)
      showToast(tt(language, 'setups.urlSourceAdded'), 'success')
    } catch (error) {
      showToast(getErrorMessage(error), 'error')
    }
  }
  ...
  async function removeSource(sourceId: string): Promise<void> {
    try {
      ...
      showToast(tt(language, 'setups.sourceRemoved'), 'success')
    } catch (error) {
      showToast(getErrorMessage(error), 'error')
    }
  }
  ...
  async function install(file: SetupFileInfo): Promise<void> {
    ...
    if (!carFolder) {
      showToast(tt(language, 'setups.selectCarFolder'), 'error')
      return
    }
    setBusy(true)
    try {
  ...
  async function toggleAutoInstall(enabled: boolean): Promise<void> {
    try {
      ...
      showToast(enabled ? tt(language, 'setups.autoInstallEnabled') : tt(language, 'setups.autoInstallDisabled'), 'success')
    } catch (error) {
      showToast(getErrorMessage(error), 'error')
    }
  }
  ...
  async function loadLibrary(showSuccess = true): Promise<void> {
    ...
    try {
      ...
      if (showSuccess) showToast(tt(language, 'setups.libraryUpdated'), 'success')
    } catch (error) {
      showToast(getErrorMessage(error), 'error')
      setLibraryItems([])
    } finally {
  ...
  async function saveMetadata(): Promise<void> {
    ...
    try {
      ...
      showToast(tt(language, 'setups.metadataSaved'), 'success')
    } catch (error) {
      showToast(getErrorMessage(error), 'error')
    }
  }
  ...
  async function compareSelected(): Promise<void> {
    if (!compareLeftPath || !compareRightPath || compareLeftPath === compareRightPath) {
      showToast(tt(language, 'setups.selectTwoDifferent'), 'error')
      return
    }
    setLibraryBusy(true)
    try {
  ...
  return (
    <div style={{ display: 'grid', gap: 16 }}>
      <div style={{ ...card, display: 'flex', justifyContent: 'space-between', gap: 16, alignItems: 'center' }}>
        <div>
          <div style={label}>{tt(language, 'setups.eyebrow')}</div>
          <h3 style={{ margin: '4px 0 0' }}>{tt(language, 'setups.title')}</h3>
          <p style={{ margin: '6px 0 0', opacity: 0.72 }}>
            {tt(language, 'setups.summary')}
          </p>
        </div>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
          <SectionExportImport sectionId="setups" label={tt(language, 'setups.exportLibrary')} onImported={() => void refreshAll()} />
          <SectionExportImport sectionId="setup-manager" label={tt(language, 'setups.exportManager')} onImported={() => void refreshAll()} />
      ...
      <div className="view-tabs" style={{ display: 'flex', gap: 8, borderBottom: '1px solid rgba(255,255,255,0.08)' }}>
        <TabButton active={activeTab === 'install'} label={tt(language, 'setups.tabInstall')} onClick={() => setActiveTab('install')} />
        <TabButton active={activeTab === 'library'} label={tt(language, 'setups.tabLibrary')} onClick={() => setActiveTab('library')} />
        <TabButton active={activeTab === 'compare'} label={tt(language, 'setups.tabCompare')} onClick={() => setActiveTab('compare')} />
      </div>
      ...
      <div style={{ display: 'grid', gap: 16 }}>
        {env && !env.supported && (
          <div style={{ ...card, borderColor: 'rgba(255,185,0,0.35)' }}>
            <strong>{tt(language, 'setups.windowsOnly')}</strong>
            <p style={{ margin: '6px 0 0', opacity: 0.78 }}>{tt(language, 'setups.expectedFolder', { folder: env.setupsDir })}</p>
          </div>
        )}
        ...
        <section style={card}>
          <div style={row}>
            <div style={{ flex: 1 }}>
              <div style={label}>{tt(language, 'setups.detectedCar')}</div>
              <strong>{detected.carName || tt(language, 'setups.noTelemetry')}</strong>
            ...
            <label style={row}>
              ...
              {tt(language, 'setups.rememberCarFolder')}
            </label>
          </div>
        </section>
        ...
        <section style={card}>
          <div style={{ ...row, justifyContent: 'space-between' }}>
            <div>
              <div style={label}>{tt(language, 'setups.sources')}</div>
              <h3 style={{ margin: '4px 0 0' }}>{tt(language, 'setups.setupLibrary')}</h3>
            </div>
            <button style={primaryButton} type="button" onClick={() => void addFolderSource()}>{tt(language, 'setups.addFolder')}</button>
          </div>
          <div style={{ ...row, marginTop: 12 }}>
            <input style={{ ...input, flex: 1, minWidth: 260 }} value={urlDraft} onChange={(event) => setUrlDraft(event.target.value)} placeholder={tt(language, 'setups.urlPlaceholder')} />
            <button style={button} type="button" onClick={() => void addUrlSource()}>{tt(language, 'setups.addUrl')}</button>
          </div>
            ...
            {config.sources.map((source) => (
              <div key={source.id} style={{ ...row, justifyContent: 'space-between', padding: 10, border: '1px solid rgba(255,255,255,0.08)', borderRadius: 'var(--radius-sm)' }}>
                <button style={{ ...button, borderColor: selectedSourceId === source.id ? 'var(--accent-primary)' : 'rgba(255,255,255,0.14)' }} type="button" onClick={() => setSelectedSourceId(source.id)}>
                  {source.kind === 'folder' ? tt(language, 'setups.folder') : 'URL'} · {source.label}
                </button>
                <small style={{ opacity: 0.62, flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{source.path ?? source.url}</small>
                <button style={button} type="button" onClick={() => void removeSource(source.id)}>{tt(language, 'common.remove')}</button>
              </div>
            ))}
            {config.sources.length === 0 && <p style={{ opacity: 0.7 }}>{tt(language, 'setups.noSources')}</p>}
          </div>
        </section>
        ...
        <section style={card}>
          <div style={row}>
            <label style={row}>
              ...
              {tt(language, 'setups.autoInstallNew')}
            </label>
            <select
              style={input}
            ...
            >
              <option value="">{tt(language, 'setups.selectFolder')}</option>
        ...
        <section style={card}>
          <div style={{ ...row, justifyContent: 'space-between' }}>
            <div>
              <div style={label}>{tt(language, 'setups.availableFiles')}</div>
              <h3 style={{ margin: '4px 0 0' }}>{selectedSource ? selectedSource.label : tt(language, 'setups.selectSource')}</h3>
            </div>
            <button disabled={!selectedSourceId || busy} style={button} type="button" onClick={() => void loadSource(selectedSourceId)}>{tt(language, 'setups.refresh')}</button>
          </div>
          <div style={{ display: 'grid', gap: 8, marginTop: 12 }}>
            {files.map((file) => (
              <div key={file.id} style={{ display: 'grid', gridTemplateColumns: '1fr minmax(180px, 260px) auto', gap: 10, alignItems: 'center', padding: 10, border: '1px solid rgba(255,255,255,0.08)', borderRadius: 'var(--radius-sm)' }}>
                <div>
                  ...
                  <div style={{ opacity: 0.6, fontSize: 12 }}>{file.sizeBytes ? `${Math.round(file.sizeBytes / 1024)} KB` : tt(language, 'setups.remote')} {file.suggestedCarFolder ? `· suggestion: ${file.suggestedCarFolder}` : ''}</div>
                </div>
                <CarFolderInput
                  language={language}
                  folders={carFolders}
                ...
                />
                <button disabled={busy} style={primaryButton} type="button" onClick={() => void install(file)}>{tt(language, 'setups.install')}</button>
              </div>
            ))}
    ...
    return (
      <div style={{ display: 'grid', gridTemplateColumns: 'minmax(320px, 1.1fr) minmax(320px, 0.9fr)', gap: 16 }}>
        <section style={card}>
          <div style={{ ...row, justifyContent: 'space-between' }}>
            <div>
              <div style={label}>{tt(language, 'setups.localLibrary')}</div>
              <h3 style={{ margin: '4px 0 0' }}>{tt(language, 'setups.indexedCount', { count: libraryItems.length })}</h3>
              <small style={{ opacity: 0.62 }}>{libraryRoot || tt(language, 'setups.folderNotLoaded')}</small>
            </div>
            <button disabled={libraryBusy} style={button} type="button" onClick={() => void loadLibrary()}>{tt(language, 'setups.refresh')}</button>
          </div>
          <div style={{ display: 'grid', gap: 8, marginTop: 12, maxHeight: 520, overflow: 'auto' }}>
            {libraryItems.map((item) => <LibraryItemButton language={language} key={item.id} active={item.path === selectedLibraryPath} item={item} onClick={() => setSelectedLibraryPath(item.path)} />)}
            {libraryItems.length === 0 && <p style={{ opacity: 0.7 }}>{libraryBusy ? 'Indexing setups…' : 'No .sto found in the local folder.'}</p>}
          </div>
        </section>

        <section style={card}>
          <div style={label}>{tt(language, 'setups.metadata')}</div>
          <h3 style={{ margin: '4px 0 12px' }}>{selectedLibraryItem?.fileName ?? tt(language, 'setups.selectSetup')}</h3>
          <MetadataEditor language={language} metadata={metadataDraft} disabled={!selectedLibraryItem} onChange={setMetadataDraft} />
          <div style={{ ...row, justifyContent: 'flex-end', marginTop: 12 }}>
            <button disabled={!selectedLibraryItem} style={primaryButton} type="button" onClick={() => void saveMetadata()}>{tt(language, 'setups.saveMetadata')}</button>
          </div>
        </section>
      </div>
    )
    ...
    return (
      <div style={{ display: 'grid', gap: 16 }}>
        <section style={card}>
          <div style={{ ...row, justifyContent: 'space-between' }}>
            <div>
              <div style={label}>{tt(language, 'setups.compareSetups')}</div>
              <h3 style={{ margin: '4px 0 0' }}>{tt(language, 'setups.deltaDiff')}</h3>
            </div>
            <button disabled={libraryBusy} style={button} type="button" onClick={() => void loadLibrary()}>{tt(language, 'setups.refreshLibrary')}</button>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr auto', gap: 10, marginTop: 12, alignItems: 'end' }}>
            <SetupSelect language={language} label={tt(language, 'setups.setupA')} items={libraryItems} value={compareLeftPath} onChange={setCompareLeftPath} />
            <SetupSelect language={language} label={tt(language, 'setups.setupB')} items={libraryItems} value={compareRightPath} onChange={setCompareRightPath} />
            <button disabled={libraryBusy || libraryItems.length < 2} style={primaryButton} type="button" onClick={() => void compareSelected()}>{tt(language, 'setups.compare')}</button>
          </div>
        </section>

        {compareResult ? <DiffView language={language} result={compareResult} /> : <section style={card}><p style={{ opacity: 0.7, margin: 0 }}>{tt(language, 'setups.selectTwoHelp')}</p></section>}
      </div>
    )
  }
}
...
function LibraryItemButton({ active, item, language, onClick }: { active: boolean; item: SetupLibraryItem; language: AppViewProps['language']; onClick(): void }): ReactElement {
  ...
  return (
    <button type="button" onClick={onClick} style={{ ...button, textAlign: 'left', borderColor: active ? 'var(--accent-primary)' : 'rgba(255,255,255,0.08)', background: active ? 'rgba(var(--accent-rgb),0.12)' : 'rgba(255,255,255,0.02)' }}>
      ...
      <div style={{ opacity: 0.72, fontSize: 12 }}>{item.metadata.car || item.carFolder || tt(language, 'setups.carNotDefined')} {item.metadata.track ? `· ${item.metadata.track}` : ''}{tags}</div>
    </button>
  )
}

function MetadataEditor({ disabled, language, metadata, onChange }: { disabled: boolean; language: AppViewProps['language']; metadata: SetupMetadata; onChange(metadata: SetupMetadata): void }): ReactElement {
  return (
    <div style={{ display: 'grid', gap: 10 }}>
      <Field label={tt(language, 'setups.car')} disabled={disabled} value={metadata.car} onChange={(value) => onChange({ ...metadata, car: value })} />
      <Field label={tt(language, 'setups.track')} disabled={disabled} value={metadata.track} onChange={(value) => onChange({ ...metadata, track: value })} />
      <Field label={tt(language, 'setups.tags')} disabled={disabled} value={metadata.tags.join(', ')} placeholder={tt(language, 'setups.tagsPlaceholder')} onChange={(value) => onChange({ ...metadata, tags: value.split(',').map((tag) => tag.trim()).filter(Boolean) })} />
      <label style={{ display: 'grid', gap: 6 }}>
        <span style={label}>{tt(language, 'setups.rating')}</span>
        <select disabled={disabled} style={input} value={metadata.rating} onChange={(event) => onChange({ ...metadata, rating: Number(event.target.value) })}>
          {[0, 1, 2, 3, 4, 5].map((rating) => <option key={rating} value={rating}>{rating === 0 ? tt(language, 'setups.noRating') : `${rating}/5`}</option>)}
        </select>
      </label>
      <label style={{ display: 'grid', gap: 6 }}>
        <span style={label}>{tt(language, 'setups.notes')}</span>
...
function SetupSelect({ items, label: selectLabel, language, onChange, value }: { items: SetupLibraryItem[]; label: string; language: AppViewProps['language']; onChange(value: string): void; value: string }): ReactElement {
  return (
    <label style={{ display: 'grid', gap: 6 }}>
      <span style={label}>{selectLabel}</span>
      <select style={input} value={value} onChange={(event) => onChange(event.target.value)}>
        <option value="">{tt(language, 'setups.selectSetup')}</option>
...
function DiffView({ language, result }: { language: AppViewProps['language']; result: SetupCompareResult }): ReactElement {
  return (
    <section style={card}>
      <div style={row}>
        <strong>{result.left.fileName}</strong>
        <span style={{ opacity: 0.58 }}>{tt(language, 'setups.vs')}</span>
        <strong>{result.right.fileName}</strong>
        <span style={{ ...warningText, marginLeft: 'auto' }}>{tt(language, 'setups.differencesCount', { count: result.diff.totalChanges })}</span>
      </div>
      <div style={{ display: 'grid', gap: 12, marginTop: 12 }}>
        {result.diff.sections.map((section) => (
          <div key={section.section} style={{ border: '1px solid rgba(255,255,255,0.08)', borderRadius: 'var(--radius-sm)', overflow: 'hidden' }}>
            ...
            <DiffEntries title={tt(language, 'setups.changed')} entries={section.changed} kind="changed" />
            <DiffEntries title={tt(language, 'setups.added')} entries={section.added} kind="added" />
            <DiffEntries title={tt(language, 'setups.removed')} entries={section.removed} kind="removed" />
          </div>
        ))}
        {result.diff.sections.length === 0 && <p style={{ opacity: 0.7, margin: 0 }}>{tt(language, 'setups.noDifferences')}</p>}
      </div>
    </section>
  )
}
...
function CarFolderInput({ folders, language, value, onChange }: { folders: string[]; language: AppViewProps['language']; value: string; onChange(value: string): void }): ReactElement {
  return (
    <div style={{ display: 'grid', gap: 6 }}>
      ...
      <input style={input} value={value} onChange={(event) => onChange(event.target.value)} placeholder={tt(language, 'setups.newCarFolder')} />
    </div>
  )
}
