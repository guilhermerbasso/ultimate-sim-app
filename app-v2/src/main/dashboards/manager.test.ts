...
describe('RC-01 full-frame preset lifecycle', () => {
  let userData: string

  beforeEach(() => {
    userData = mkdtempSync(join(process.cwd(), 'dashboard-racecon-rc01-test-'))
  })
  ...
  it('creates through IPC, persists, lazily materializes, and opens the RC-01 preset', async () => {
    ...
    const created = await create!({}, 'racecon_rc01_dash') as { id: string; name: string }
    expect(created.name).toContain('RaceCon RC-01 Apex Strike')
    expect(manager.getDashboard(created.id)?.elements).toMatchObject([
      { type: 'overlaywidget', widgetId: 'raceconRc01Dash' }
    ])
    ...
    const opened = manager.openWindow('racecon_rc01_dash', { displayId: primaryDisplay.id, fullscreen: true })
    await vi.waitFor(() => expect(window).toBeDefined())
    window.finishLoad()
    await expect(opened).resolves.toMatchObject({ id: 'racecon_rc01_dash', displayId: primaryDisplay.id })
    expect(manager.getDashboard('racecon_rc01_dash')?.elements).toMatchObject([
      { type: 'overlaywidget', widgetId: 'raceconRc01Dash' }
    ])
    expect(existsSync(join(userData, 'dashboards', 'racecon_rc01_dash.json'))).toBe(true)
    expect(window.options.title).toContain('RaceCon RC-01 Apex Strike')
  })
})

describe('RC-02 full-frame preset lifecycle', () => {
  let userData: string

  beforeEach(() => {
    userData = mkdtempSync(join(process.cwd(), 'dashboard-racecon-rc02-test-'))
  })
  ...
  it('creates through IPC, persists, lazily materializes, and opens the RC-02 preset', async () => {
    // Keep a non-empty store so lazy materialization is exercised rather than first-run seeding.
    persistDashboard(userData, raceTrafficAttack())
    const handlers = new Map<string, IpcHandler>()
    const manager = makeHeadlessManager(userData, handlers)
    ...
    const create = handlers.get('app:dash:createPreset')
    expect(create).toBeDefined()
    const created = await create!({}, 'racecon_rc02_dash') as { id: string; name: string }
    expect(created.name).toContain('RaceCon RC-02 Purple Lap')
    expect(manager.getDashboard(created.id)?.elements).toMatchObject([
      { type: 'overlaywidget', widgetId: 'raceconRc02Dash' }
    ])
    expect(existsSync(join(userData, 'dashboards', `${created.id}.json`))).toBe(true)
    ...
    const opened = manager.openWindow('racecon_rc02_dash', { displayId: primaryDisplay.id, fullscreen: true })
    await vi.waitFor(() => expect(window).toBeDefined())
    window.finishLoad()
    await expect(opened).resolves.toMatchObject({ id: 'racecon_rc02_dash', displayId: primaryDisplay.id })
    expect(manager.getDashboard('racecon_rc02_dash')?.elements).toMatchObject([
      { type: 'overlaywidget', widgetId: 'raceconRc02Dash' }
    ])
    expect(existsSync(join(userData, 'dashboards', 'racecon_rc02_dash.json'))).toBe(true)
    expect(window.options.title).toContain('RaceCon RC-02 Purple Lap')
  })
})

describe('RC-03 full-frame preset lifecycle', () => {
  let userData: string

  beforeEach(() => {
    userData = mkdtempSync(join(process.cwd(), 'dashboard-racecon-rc03-test-'))
  })
  ...
  it('creates through IPC, persists, lazily materializes, and opens the RC-03 preset', async () => {
    // Keep a non-empty store so lazy materialization is exercised rather than first-run seeding.
    persistDashboard(userData, raceTrafficAttack())
    const handlers = new Map<string, IpcHandler>()
    const manager = makeHeadlessManager(userData, handlers)
    ...
    const create = handlers.get('app:dash:createPreset')
    expect(create).toBeDefined()
    const created = await create!({}, 'racecon_rc03_dash') as { id: string; name: string }
    expect(created.name).toContain('RaceCon RC-03 Long Night')
    expect(manager.getDashboard(created.id)?.elements).toMatchObject([
      { type: 'overlaywidget', widgetId: 'raceconRc03Dash' }
    ])
    expect(existsSync(join(userData, 'dashboards', `${created.id}.json`))).toBe(true)
    ...
    const opened = manager.openWindow('racecon_rc03_dash', { displayId: primaryDisplay.id, fullscreen: true })
    await vi.waitFor(() => expect(window).toBeDefined())
    window.finishLoad()
    await expect(opened).resolves.toMatchObject({ id: 'racecon_rc03_dash', displayId: primaryDisplay.id })
    expect(manager.getDashboard('racecon_rc03_dash')?.elements).toMatchObject([
      { type: 'overlaywidget', widgetId: 'raceconRc03Dash' }
    ])
    expect(existsSync(join(userData, 'dashboards', 'racecon_rc03_dash.json'))).toBe(true)
    expect(window.options.title).toContain('RaceCon RC-03 Long Night')
  })
})

describe('RC-04 full-frame preset lifecycle', () => {
  let userData: string

  beforeEach(() => {
    userData = mkdtempSync(join(process.cwd(), 'dashboard-racecon-rc04-test-'))
  })
  ...
  it('creates through IPC, persists, lazily materializes, and opens the RC-04 preset', async () => {
    // Keep a non-empty store so lazy materialization is exercised rather than first-run seeding.
    persistDashboard(userData, raceTrafficAttack())
    const handlers = new Map<string, IpcHandler>()
    const manager = makeHeadlessManager(userData, handlers)
    ...
    const create = handlers.get('app:dash:createPreset')
    expect(create).toBeDefined()
    const created = await create!({}, 'racecon_rc04_dash') as { id: string; name: string }
    expect(created.name).toContain('RaceCon RC-04 Box Now')
    expect(manager.getDashboard(created.id)?.elements).toMatchObject([
      { type: 'overlaywidget', widgetId: 'raceconRc04Dash' }
    ])
    expect(existsSync(join(userData, 'dashboards', `${created.id}.json`))).toBe(true)
    ...
    const opened = manager.openWindow('racecon_rc04_dash', { displayId: primaryDisplay.id, fullscreen: true })
    await vi.waitFor(() => expect(window).toBeDefined())
    window.finishLoad()
    await expect(opened).resolves.toMatchObject({ id: 'racecon_rc04_dash', displayId: primaryDisplay.id })
    expect(manager.getDashboard('racecon_rc04_dash')?.elements).toMatchObject([
      { type: 'overlaywidget', widgetId: 'raceconRc04Dash' }
    ])
    expect(existsSync(join(userData, 'dashboards', 'racecon_rc04_dash.json'))).toBe(true)
    expect(window.options.title).toContain('RaceCon RC-04 Box Now')
  })
})
...
describe('RC-07 full-frame preset lifecycle', () => {
  let userData: string

  beforeEach(() => {
    userData = mkdtempSync(join(process.cwd(), 'dashboard-racecon-rc07-test-'))
  })
  ...
  it('creates through IPC, persists, lazily materializes, and opens the RC-07 preset', async () => {
    // Keep a non-empty store so lazy materialization is exercised rather than first-run seeding.
    persistDashboard(userData, raceTrafficAttack())
    const handlers = new Map<string, IpcHandler>()
    const manager = makeHeadlessManager(userData, handlers)
    ...
    const create = handlers.get('app:dash:createPreset')
    expect(create).toBeDefined()
    const created = await create!({}, 'racecon_rc07_dash') as { id: string; name: string }
    expect(created.name).toContain('RaceCon RC-07 Blue Flags')
    expect(manager.getDashboard(created.id)?.elements).toMatchObject([
      { type: 'overlaywidget', widgetId: 'raceconRc07Dash' }
    ])
    expect(existsSync(join(userData, 'dashboards', `${created.id}.json`))).toBe(true)
    ...
    const opened = manager.openWindow('racecon_rc07_dash', { displayId: primaryDisplay.id, fullscreen: true })
    await vi.waitFor(() => expect(window).toBeDefined())
    window.finishLoad()
    await expect(opened).resolves.toMatchObject({ id: 'racecon_rc07_dash', displayId: primaryDisplay.id })
    expect(manager.getDashboard('racecon_rc07_dash')?.elements).toMatchObject([
      { type: 'overlaywidget', widgetId: 'raceconRc07Dash' }
    ])
    expect(existsSync(join(userData, 'dashboards', 'racecon_rc07_dash.json'))).toBe(true)
    expect(window.options.title).toContain('RaceCon RC-07 Blue Flags')
  })
})

describe('RC-08 full-frame preset lifecycle', () => {
  let userData: string

  beforeEach(() => {
    userData = mkdtempSync(join(process.cwd(), 'dashboard-racecon-rc08-test-'))
  })
  ...
  it('creates through IPC, persists, lazily materializes, and opens the RC-08 preset', async () => {
    // Keep a non-empty store so lazy materialization is exercised rather than first-run seeding.
    persistDashboard(userData, raceTrafficAttack())
    const handlers = new Map<string, IpcHandler>()
    const manager = makeHeadlessManager(userData, handlers)
    ...
    const create = handlers.get('app:dash:createPreset')
    expect(create).toBeDefined()
    const created = await create!({}, 'racecon_rc08_dash') as { id: string; name: string }
    expect(created.name).toContain('RaceCon RC-08 Rain Line')
    expect(manager.getDashboard(created.id)?.elements).toMatchObject([
      { type: 'overlaywidget', widgetId: 'raceconRc08Dash' }
    ])
    expect(existsSync(join(userData, 'dashboards', `${created.id}.json`))).toBe(true)
    ...
    const opened = manager.openWindow('racecon_rc08_dash', { displayId: primaryDisplay.id, fullscreen: true })
    await vi.waitFor(() => expect(window).toBeDefined())
    window.finishLoad()
    await expect(opened).resolves.toMatchObject({ id: 'racecon_rc08_dash', displayId: primaryDisplay.id })
    expect(manager.getDashboard('racecon_rc08_dash')?.elements).toMatchObject([
      { type: 'overlaywidget', widgetId: 'raceconRc08Dash' }
    ])
    expect(existsSync(join(userData, 'dashboards', 'racecon_rc08_dash.json'))).toBe(true)
    expect(window.options.title).toContain('RaceCon RC-08 Rain Line')
  })
})

describe('RC-09 full-frame preset lifecycle', () => {
  let userData: string

  beforeEach(() => {
    userData = mkdtempSync(join(process.cwd(), 'dashboard-racecon-rc09-test-'))
  })
  ...
  it('creates through IPC, persists, lazily materializes, and opens the RC-09 preset', async () => {
    // Keep a non-empty store so lazy materialization is exercised rather than first-run seeding.
    persistDashboard(userData, raceTrafficAttack())
    const handlers = new Map<string, IpcHandler>()
    const manager = makeHeadlessManager(userData, handlers)
    ...
    const create = handlers.get('app:dash:createPreset')
    expect(create).toBeDefined()
    const created = await create!({}, 'racecon_rc09_dash') as { id: string; name: string }
    expect(created.name).toContain('RaceCon RC-09 Stage Time')
    expect(manager.getDashboard(created.id)?.elements).toMatchObject([
      { type: 'overlaywidget', widgetId: 'raceconRc09Dash' }
    ])
    expect(existsSync(join(userData, 'dashboards', `${created.id}.json`))).toBe(true)
    ...
    const opened = manager.openWindow('racecon_rc09_dash', { displayId: primaryDisplay.id, fullscreen: true })
    await vi.waitFor(() => expect(window).toBeDefined())
    window.finishLoad()
    await expect(opened).resolves.toMatchObject({ id: 'racecon_rc09_dash', displayId: primaryDisplay.id })
    expect(manager.getDashboard('racecon_rc09_dash')?.elements).toMatchObject([
      { type: 'overlaywidget', widgetId: 'raceconRc09Dash' }
    ])
    expect(existsSync(join(userData, 'dashboards', 'racecon_rc09_dash.json'))).toBe(true)
    expect(window.options.title).toContain('RaceCon RC-09 Stage Time')
  })
})

describe('RC-10 full-frame preset lifecycle', () => {
  let userData: string

  beforeEach(() => {
    userData = mkdtempSync(join(process.cwd(), 'dashboard-racecon-rc10-test-'))
  })
  ...
  it('creates through IPC, persists, lazily materializes, and opens the RC-10 preset', async () => {
    // Keep a non-empty store so lazy materialization is exercised rather than first-run seeding.
    persistDashboard(userData, raceTrafficAttack())
    const handlers = new Map<string, IpcHandler>()
    const manager = makeHeadlessManager(userData, handlers)
    ...
    const create = handlers.get('app:dash:createPreset')
    expect(create).toBeDefined()
    const created = await create!({}, 'racecon_rc10_dash') as { id: string; name: string }
    expect(created.name).toContain('RaceCon RC-10 Clear Sight')
    expect(manager.getDashboard(created.id)?.elements).toMatchObject([
      { type: 'overlaywidget', widgetId: 'raceconRc10Dash' }
    ])
    expect(existsSync(join(userData, 'dashboards', `${created.id}.json`))).toBe(true)
    ...
    const opened = manager.openWindow('racecon_rc10_dash', { displayId: primaryDisplay.id, fullscreen: true })
    await vi.waitFor(() => expect(window).toBeDefined())
    window.finishLoad()
    await expect(opened).resolves.toMatchObject({ id: 'racecon_rc10_dash', displayId: primaryDisplay.id })
    expect(manager.getDashboard('racecon_rc10_dash')?.elements).toMatchObject([
      { type: 'overlaywidget', widgetId: 'raceconRc10Dash' }
    ])
    expect(existsSync(join(userData, 'dashboards', 'racecon_rc10_dash.json'))).toBe(true)
    expect(window.options.title).toContain('RaceCon RC-10 Clear Sight')
  })
})

describe('RC-11 full-frame preset lifecycle', () => {
  let userData: string

  beforeEach(() => {
    userData = mkdtempSync(join(process.cwd(), 'dashboard-racecon-rc11-test-'))
  })
  ...
  it('creates through IPC, persists, lazily materializes, and opens the RC-11 preset', async () => {
    // Keep a non-empty store so lazy materialization is exercised rather than first-run seeding.
    persistDashboard(userData, raceTrafficAttack())
    const handlers = new Map<string, IpcHandler>()
    const manager = makeHeadlessManager(userData, handlers)
    ...
    const create = handlers.get('app:dash:createPreset')
    expect(create).toBeDefined()
    const created = await create!({}, 'racecon_rc11_dash') as { id: string; name: string }
    expect(created.name).toContain('RaceCon RC-11 Trace Room')
    expect(manager.getDashboard(created.id)?.elements).toMatchObject([
      { type: 'overlaywidget', widgetId: 'raceconRc11Dash' }
    ])
    expect(existsSync(join(userData, 'dashboards', `${created.id}.json`))).toBe(true)
    ...
    const opened = manager.openWindow('racecon_rc11_dash', { displayId: primaryDisplay.id, fullscreen: true })
    await vi.waitFor(() => expect(window).toBeDefined())
    window.finishLoad()
    await expect(opened).resolves.toMatchObject({ id: 'racecon_rc11_dash', displayId: primaryDisplay.id })
    expect(manager.getDashboard('racecon_rc11_dash')?.elements).toMatchObject([
      { type: 'overlaywidget', widgetId: 'raceconRc11Dash' }
    ])
    expect(existsSync(join(userData, 'dashboards', 'racecon_rc11_dash.json'))).toBe(true)
    expect(window.options.title).toContain('RaceCon RC-11 Trace Room')
  })
})
...
describe('RC-12 full-frame preset lifecycle', () => {
  ...
  beforeEach(() => {
    userData = mkdtempSync(join(process.cwd(), 'dashboard-racecon-rc12-test-'))
  })
  ...
  it('creates through IPC, persists, lazily materializes, and opens the RC-12 preset', async () => {
    // Keep a non-empty store so lazy materialization is exercised rather than first-run seeding.
    persistDashboard(userData, raceTrafficAttack())
    const handlers = new Map<string, IpcHandler>()
    const manager = makeHeadlessManager(userData, handlers)
    ...
    const create = handlers.get('app:dash:createPreset')
    expect(create).toBeDefined()
    const created = await create!({}, 'racecon_rc12_dash') as { id: string; name: string }
    expect(created.name).toContain('RaceCon RC-12 On Air')
    expect(manager.getDashboard(created.id)?.elements).toMatchObject([
      { type: 'overlaywidget', widgetId: 'raceconRc12Dash' }
    ])
    expect(existsSync(join(userData, 'dashboards', `${created.id}.json`))).toBe(true)
    ...
    const opened = manager.openWindow('racecon_rc12_dash', { displayId: primaryDisplay.id, fullscreen: true })
    await vi.waitFor(() => expect(window).toBeDefined())
    window.finishLoad()
    await expect(opened).resolves.toMatchObject({ id: 'racecon_rc12_dash', displayId: primaryDisplay.id })
    expect(manager.getDashboard('racecon_rc12_dash')?.elements).toMatchObject([
      { type: 'overlaywidget', widgetId: 'raceconRc12Dash' }
    ])
    expect(existsSync(join(userData, 'dashboards', 'racecon_rc12_dash.json'))).toBe(true)
    expect(window.options.title).toContain('RaceCon RC-12 On Air')
  })
})
