...
describe('RaceCon mock telemetry', () => {
  ...
  it('models the pit sequence as gated phases', () => {
    ...
    expect(limiter.snapshot.rpm).toBeLessThan(5000)
    expect(limiter.snapshot.throttle).toBeLessThan(0.5)
    expect(limiter.snapshot.brake).toBe(0)
    expect(limiter.snapshot.absActive).toBe(false)
    expect(limiter.snapshot.absCutPct).toBe(0)
    expect(limiter.snapshot.longAccelG).toBeGreaterThan(0)
    ...
    expect(service.snapshot.rpm).toBeGreaterThanOrEqual(800)
    expect(service.snapshot.rpm).toBeLessThanOrEqual(1200)
    expect(service.snapshot.shiftIndicatorPct).toBe(0)
    expect(release.phase).toBe('pit-release')
    expect(release.snapshot.absActive).toBe(false)
    expect(release.snapshot.absCutPct).toBe(0)
    expect(release.snapshot.longAccelG).toBeGreaterThan(0)
  })
