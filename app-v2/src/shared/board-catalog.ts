...
export const BOARD_CATALOG: Record<KnownPinoutBoardId, BoardCatalogEntry> = {
  uno: { id: 'uno', deviceBoardId: 'uno', name: 'Arduino Uno R3', mcu: 'ATmega328P', fqbn: 'arduino:avr:uno', voltage: '5V', usbHid: false, notes: 'Beginner-friendly 5V board. D0/D1 are USB serial; prefer avoiding them. A4/A5 are the I2C bus.', pins: [...unoDigital, ...unoAnalog, ...powerPins()] },
  'uno-r4-minima': { id: 'uno-r4-minima', name: 'Arduino Uno R4 Minima', mcu: 'RA4M1', fqbn: 'arduino:renesas_uno:minima', voltage: '5V', usbHid: true, notes: 'Modern Uno-size board with 5V logic and native USB. Pin layout follows Uno R3; DAC/CAN features are not modeled here.', pins: [...unoDigital.map((pin) => ({ ...pin, interrupt: pin.digital || pin.interrupt })), ...unoAnalog, ...powerPins()] },
  'uno-r4-wifi': { id: 'uno-r4-wifi', name: 'Arduino Uno R4 WiFi', mcu: 'RA4M1', fqbn: 'arduino:renesas_uno:unor4wifi', voltage: '5V', usbHid: true, notes: 'Uno R4 with Wi-Fi coprocessor and LED matrix. Uses the same beginner pin labels as Uno R3 for shields and wiring.', pins: [...unoDigital.map((pin) => ({ ...pin, interrupt: pin.digital || pin.interrupt })), ...unoAnalog, ...powerPins()] },
  nano: { id: 'nano', deviceBoardId: 'nano', name: 'Arduino Nano', mcu: 'ATmega328P', fqbn: 'arduino:avr:nano', voltage: '5V', usbHid: false, notes: 'Tiny Uno-style board for compact boxes. A6/A7 are analog input only. A4/A5 are I2C.', pins: [...withUart(avrDigital(13, ['D3', 'D5', 'D6', 'D9', 'D10', 'D11'], ['D2', 'D3'], { D10: 'ss', D11: 'mosi', D12: 'miso', D13: 'sck' }), ['D0'], ['D1']), ...avrAnalog(8, { A4: 'sda', A5: 'scl' }, 6), ...powerPins()] },
  'nano-every': { id: 'nano-every', name: 'Arduino Nano Every', mcu: 'ATmega4809', fqbn: 'arduino:megaavr:nona4809', voltage: '5V', usbHid: false, notes: 'Modern Nano-sized board. D3/D5/D6/D9/D10 PWM, A4/A5 I2C. Good replacement for classic Nano.', pins: [...withUart(avrDigital(13, ['D3', 'D5', 'D6', 'D9', 'D10'], ['D2', 'D3']), ['D0'], ['D1']), ...avrAnalog(8, { A4: 'sda', A5: 'scl' }), ...powerPins()] },
  'nano-33-iot': { id: 'nano-33-iot', name: 'Arduino Nano 33 IoT', mcu: 'SAMD21G18A', fqbn: 'arduino:samd:nano_33_iot', voltage: '3.3V', usbHid: true, notes: 'Nano form factor with native USB, Wi-Fi and 3.3V logic. Do not feed 5V signals into GPIO pins.', pins: nano33IotPins },
  'pro-mini-3v3': { id: 'pro-mini-3v3', name: 'Arduino Pro Mini 3.3V / 8 MHz', mcu: 'ATmega328P', fqbn: 'arduino:avr:pro:cpu=8MHzatmega328', voltage: '3.3V', usbHid: false, notes: 'Small breadboard board without onboard USB. Needs an FTDI/USB-serial adapter for upload.', pins: [...unoDigital, ...unoAnalog, ...powerPins(true, false)] },
  'pro-mini-5v': { id: 'pro-mini-5v', name: 'Arduino Pro Mini 5V / 16 MHz', mcu: 'ATmega328P', fqbn: 'arduino:avr:pro:cpu=16MHzatmega328', voltage: '5V', usbHid: false, notes: 'Small 5V board without onboard USB. Needs an FTDI/USB-serial adapter for upload.', pins: [...unoDigital, ...unoAnalog, ...powerPins()] },
  promicro: { id: 'promicro', deviceBoardId: 'pro-micro', name: 'Arduino Pro Micro', mcu: 'ATmega32U4', fqbn: 'arduino:avr:micro', voltage: '5V/3.3V', usbHid: true, notes: 'Native USB HID board, excellent for button boxes. D2=SDA and D3=SCL; board voltage depends on the model you buy.', pins: [...atmega32u4Digital.filter((pin) => !['D11', 'D12', 'D13'].includes(pin.pin)), ...atmega32u4Analog.filter((pin) => !['A11'].includes(pin.pin)), p('VCC', { digital: false, power: '5v', notes: 'VCC follows the board version: 5V or 3.3V.' }), p('RAW', { digital: false, power: 'vin' }), p('GND', { digital: false, power: 'gnd' }), p('RST', { digital: false, power: 'reset' })] },
  leonardo: { id: 'leonardo', deviceBoardId: 'leonardo', name: 'Arduino Leonardo', mcu: 'ATmega32U4', fqbn: 'arduino:avr:leonardo', voltage: '5V', usbHid: true, notes: 'Uno-sized native USB HID board. Great for joystick/button box firmware because it can appear as a USB device.', pins: [...atmega32u4Digital, ...atmega32u4Analog, ...powerPins()] },
  micro: { id: 'micro', name: 'Arduino Micro', mcu: 'ATmega32U4', fqbn: 'arduino:avr:micro', voltage: '5V', usbHid: true, notes: 'Small native USB HID board with Leonardo-style pin capabilities.', pins: [...atmega32u4Digital, ...atmega32u4Analog, ...powerPins()] },
  mega: { id: 'mega', deviceBoardId: 'mega2560', name: 'Arduino Mega 2560', mcu: 'ATmega2560', fqbn: 'arduino:avr:mega', voltage: '5V', usbHid: false, notes: 'Best 5V choice when you need many direct pins. I2C is D20/D21; SPI is D50-D53.', pins: [...withUart(avrDigital(53, ['D2', 'D3', 'D4', 'D5', 'D6', 'D7', 'D8', 'D9', 'D10', 'D11', 'D12', 'D13', 'D44', 'D45', 'D46'], ['D2', 'D3', 'D18', 'D19', 'D20', 'D21'], { D50: 'miso', D51: 'mosi', D52: 'sck', D53: 'ss' }).map((pin) => pin.pin === 'D20' ? { ...pin, i2c: 'sda' as const, notes: 'SDA for I2C.' } : pin.pin === 'D21' ? { ...pin, i2c: 'scl' as const, notes: 'SCL for I2C.' } : pin), ['D0', 'D19', 'D17', 'D15'], ['D1', 'D18', 'D16', 'D14']), ...avrAnalog(16), ...powerPins()] },
  due: { id: 'due', name: 'Arduino Due', mcu: 'SAM3X8E', fqbn: 'arduino:sam:arduino_due_x', voltage: '3.3V', usbHid: true, notes: 'Large 3.3V ARM board. Most digital pins support interrupts; PWM on D2-D13. Do not connect 5V signals.', pins: [...withUart(avrDigital(53, ['D2', 'D3', 'D4', 'D5', 'D6', 'D7', 'D8', 'D9', 'D10', 'D11', 'D12', 'D13'], Array.from({ length: 54 }, (_, i) => `D${i}`), { D52: 'miso', D51: 'mosi', D53: 'sck', D10: 'ss' }).map((pin) => pin.pin === 'D20' ? { ...pin, i2c: 'sda' as const } : pin.pin === 'D21' ? { ...pin, i2c: 'scl' as const } : pin), ['D0', 'D19', 'D17', 'D15'], ['D1', 'D18', 'D16', 'D14']), ...avrAnalog(12), ...powerPins(true, false)] },
  zero: { id: 'zero', name: 'Arduino Zero', mcu: 'SAMD21G18A', fqbn: 'arduino:samd:arduino_zero_native', voltage: '3.3V', usbHid: true, notes: 'Uno-sized SAMD board with native USB and 3.3V logic. Great for compact HID projects.', pins: zeroPins },
  'mkr-wifi-1010': { id: 'mkr-wifi-1010', name: 'Arduino MKR WiFi 1010', mcu: 'SAMD21G18A', fqbn: 'arduino:samd:mkrwifi1010', voltage: '3.3V', usbHid: true, notes: 'MKR 3.3V board with Wi-Fi; compact for wireless telemetry accessories.', pins: mkrPins },
  'mkr-zero': { id: 'mkr-zero', name: 'Arduino MKR Zero', mcu: 'SAMD21G18A', fqbn: 'arduino:samd:mkrzero', voltage: '3.3V', usbHid: true, notes: 'MKR 3.3V board with native USB and microSD; useful when you do not need wireless.', pins: mkrPins },
  mkr1000: { id: 'mkr1000', name: 'Arduino MKR1000 WiFi', mcu: 'SAMD21G18A', fqbn: 'arduino:samd:mkr1000', voltage: '3.3V', usbHid: true, notes: 'Older MKR Wi-Fi board. Keep GPIO at 3.3V.', pins: mkrPins },
  'mkr-wan-1310': { id: 'mkr-wan-1310', name: 'Arduino MKR WAN 1310', mcu: 'SAMD21G18A', fqbn: 'arduino:samd:mkrwan1310', voltage: '3.3V', usbHid: true, notes: 'LoRa MKR board; pinout is useful for export/documentation even if button-box firmware rarely needs LoRa.', pins: mkrPins },
  'mkr-gsm-1400': { id: 'mkr-gsm-1400', name: 'Arduino MKR GSM 1400', mcu: 'SAMD21G18A', fqbn: 'arduino:samd:mkrgsm1400', voltage: '3.3V', usbHid: true, notes: 'Cellular MKR board; cataloged for wiring/export completeness.', pins: mkrPins },
  'mkr-nb-1500': { id: 'mkr-nb-1500', name: 'Arduino MKR NB 1500', mcu: 'SAMD21G18A', fqbn: 'arduino:samd:mkrnb1500', voltage: '3.3V', usbHid: true, notes: 'NB-IoT/LTE-M MKR board; cataloged for wiring/export completeness.', pins: mkrPins },
  esp32: { id: 'esp32', deviceBoardId: 'esp32', name: 'ESP32 DevKit', mcu: 'ESP32', fqbn: 'esp32:esp32:esp32', voltage: '3.3V', usbHid: false, notes: '3.3V logic, Wi-Fi/Bluetooth. GPIO34-39 are input-only. GPIO6-11 are reserved for flash and intentionally omitted.', pins: esp32Pins },
  esp32s2: { id: 'esp32s2', name: 'ESP32-S2 DevKit', mcu: 'ESP32-S2', fqbn: 'esp32:esp32:esp32s2', voltage: '3.3V', usbHid: true, notes: 'Single-core ESP32 with native USB. Board variants differ; avoid pins marked boot/special unless you know the silkscreen.', pins: uniquePins(esp32s2Pins) },
  esp32s3: { id: 'esp32s3', deviceBoardId: 'esp32s3', name: 'ESP32-S3 DevKit', mcu: 'ESP32-S3', fqbn: 'esp32:esp32:esp32s3', voltage: '3.3V', usbHid: true, notes: 'Native USB and many GPIOs. DevKit pinouts vary by vendor; verify the silkscreen before soldering.', pins: esp32s3Pins },
  esp32c3: { id: 'esp32c3', name: 'ESP32-C3 DevKit', mcu: 'ESP32-C3', fqbn: 'esp32:esp32:esp32c3', voltage: '3.3V', usbHid: true, notes: 'RISC-V ESP32 with native USB on many boards. Fewer pins than ESP32/S3; check boot straps.', pins: esp32c3Pins },
  'esp8266-nodemcu': { id: 'esp8266-nodemcu', deviceBoardId: 'esp8266', name: 'ESP8266 NodeMCU v2', mcu: 'ESP8266', fqbn: 'esp8266:esp8266:nodemcuv2', voltage: '3.3V', usbHid: false, notes: 'Wi-Fi board with one analog input. GPIO6-11 are flash pins and omitted. Not suitable for native USB HID.', pins: esp8266NodeMcuPins },
  'esp8266-d1-mini': { id: 'esp8266-d1-mini', deviceBoardId: 'esp8266', name: 'Wemos D1 mini / ESP8266', mcu: 'ESP8266', fqbn: 'esp8266:esp8266:d1_mini', voltage: '3.3V', usbHid: false, notes: 'Tiny ESP8266 board; great for Wi-Fi accessories, not native USB HID. Uses the common D0-D8 labels.', pins: esp8266NodeMcuPins },
  teensy40: { id: 'teensy40', name: 'Teensy 4.0', mcu: 'IMXRT1062', fqbn: 'teensy:avr:teensy40', voltage: '3.3V', usbHid: true, notes: 'Very fast native USB board. Auto-flash is export-only in this app unless Teensy tools are installed separately.', pins: teensy40Pins },
  teensy41: { id: 'teensy41', name: 'Teensy 4.1', mcu: 'IMXRT1062', fqbn: 'teensy:avr:teensy41', voltage: '3.3V', usbHid: true, notes: 'Large fast native USB board with many pins. Auto-flash is export-only in this app unless Teensy tools are installed separately.', pins: teensy41Pins },
  pico: { id: 'pico', name: 'Raspberry Pi Pico / RP2040', mcu: 'RP2040', fqbn: 'arduino:mbed_rp2040:pico', voltage: '3.3V', usbHid: true, notes: 'Low-cost RP2040 board with native USB. GPIO pins are 3.3V only; analog inputs are GPIO26-GPIO28.', pins: picoPins }
}

const powerCommon = ['Connect GND from the component to board GND.', 'Use the voltage listed in Power needs; do not mix 5V signals into 3.3V-only boards without level shifting.']
...
export const PINOUT_COMPONENT_LIBRARY: PinoutComponentDefinition[] = [
  ...
  { id: 'slide-pot', type: 'control', name: 'Slide potentiometer', shortName: 'Slide pot', description: 'Linear analog slider for volume, brake bias or trim.', plainLanguageDescription: 'A slider that reports a smooth position.', category: 'Inputs', icon: '▰', iconHint: 'linear slider', defaultLabel: 'Slider axis', protocolKey: 'control.slidePot', power: ['5V', '3V3', 'GND'], roles: [{ role: 'signal', label: 'Wiper / signal', kind: 'analog', muxCapable: true }], defaultWiringNotes: ['Outer legs go to power and GND; center/wiper goes to an analog input.', 'Use the board logic voltage for the high side.'] },
  ...
  { id: 'ds18b20', type: 'control', name: 'DS18B20 temperature sensor', shortName: 'DS18B20', description: '1-Wire digital temperature probe.', plainLanguageDescription: 'A digital temperature probe using one data pin.', category: 'Sensors', icon: '♨', iconHint: 'temperature probe', defaultLabel: 'DS18B20', protocolKey: 'sensor.ds18b20', power: ['5V', '3V3', 'GND'], roles: [{ role: 'data', label: '1-Wire DATA', kind: 'digital' }], defaultWiringNotes: ['DATA needs a 4.7k pull-up to the sensor voltage.', 'Multiple DS18B20 sensors can share one data wire if firmware supports addresses.'] },
  { id: 'lm35-temperature', type: 'control', name: 'LM35 analog temperature sensor', shortName: 'LM35', description: 'Analog temperature sensor with voltage proportional to Celsius.', plainLanguageDescription: 'A temperature sensor read by an analog input.', category: 'Sensors', icon: '♨', iconHint: 'analog temperature sensor', defaultLabel: 'LM35', protocolKey: 'sensor.lm35', power: ['5V', 'GND'], roles: [{ role: 'signal', label: 'Analog out', kind: 'analog', muxCapable: true }], defaultWiringNotes: ['OUT goes to an analog input.', 'Check voltage range before using with 3.3V boards.'] },
  ...
  { id: 'ina219-current', type: 'control', name: 'INA219 current/voltage sensor', shortName: 'INA219', description: 'I2C high-side current and bus-voltage monitor.', plainLanguageDescription: 'Measures current and voltage using the I2C bus.', category: 'Sensors', icon: 'A', iconHint: 'current sensor', defaultLabel: 'Current sensor', protocolKey: 'sensor.ina219', power: ['3V3', '5V', 'GND'], roles: [{ role: 'sda', label: 'SDA', kind: 'i2c' }, { role: 'scl', label: 'SCL', kind: 'i2c' }], defaults: { address: '0x40' }, defaultWiringNotes: ['SDA/SCL share the I2C bus.', 'Wire the load current path through VIN+/VIN- per the module rating.'] },
  { id: 'acs712-current', type: 'control', name: 'ACS712 analog current sensor', shortName: 'ACS712', description: 'Hall-effect current sensor with analog output.', plainLanguageDescription: 'Measures current and outputs a voltage for an analog input.', category: 'Sensors', icon: 'A', iconHint: 'analog current sensor', defaultLabel: 'Analog current sensor', protocolKey: 'sensor.acs712', power: ['5V', 'GND'], roles: [{ role: 'signal', label: 'Analog OUT', kind: 'analog', muxCapable: true }], defaultWiringNotes: ['OUT goes to an analog input.', 'Use only within the module current and isolation ratings.'] },
  ...
  { id: 'drv8825-stepper-driver', type: 'gauge', name: 'DRV8825 stepper driver', shortName: 'DRV8825', description: 'Step/dir driver for bipolar stepper motors with higher voltage support.', plainLanguageDescription: 'A stronger stepper driver controlled by STEP and DIR pins.', category: 'Motors', icon: '◷', iconHint: 'stepper driver', defaultLabel: 'DRV8825 driver', protocolKey: 'motor.stepper.drv8825', power: ['VIN', '5V', 'GND'], roles: [{ role: 'step', label: 'STEP', kind: 'digital' }, { role: 'dir', label: 'DIR', kind: 'digital' }, { role: 'en', label: 'EN enable', kind: 'digital', optional: true }], defaultWiringNotes: ['Set current limit and microstep pins per your motor.', 'Use external motor power and common GND.'] },
  ...
  { id: 'buck-converter', type: 'power', name: 'DC-DC buck converter', shortName: 'Buck converter', description: 'Step-down regulator for making 5V/3.3V rails from a higher voltage.', plainLanguageDescription: 'A regulator that turns a higher battery/load voltage into a lower rail.', category: 'Power', icon: '▤', iconHint: 'regulator module', defaultLabel: 'Buck converter', protocolKey: 'power.regulator.buck', power: ['VIN', '5V', '3V3', 'GND'], roles: [], exportOnly: true, defaultWiringNotes: ['Set output voltage before connecting electronics.', 'Feeds 5V/3V3 rails or VIN depending on the board and regulator.', 'This is power wiring only, not a GPIO component.'] },
  { id: 'boost-converter', type: 'power', name: 'DC-DC boost converter', shortName: 'Boost converter', description: 'Step-up regulator for raising battery voltage.', plainLanguageDescription: 'A regulator that turns a low battery voltage into a higher rail.', category: 'Power', icon: '▤', iconHint: 'boost regulator', defaultLabel: 'Boost converter', protocolKey: 'power.regulator.boost', power: ['VIN', '5V', 'GND'], roles: [], exportOnly: true, defaultWiringNotes: ['Set output voltage and verify current capability before connecting the board/load.', 'Use for rails like 5V from a 1S battery when current is adequate.', 'No MCU signal pins are required.'] },
...
function getPowerCompatibility(component: PinoutComponentDefinition, board: BoardCatalogEntry): { reasons: string[]; warnings: string[] } {
  const reasons: string[] = []
  const warnings: string[] = []
  const voltageRails = component.power.filter((rail): rail is '5V' | '3V3' => rail === '5V' || rail === '3V3')
  if (voltageRails.length > 0 && !voltageRails.some((rail) => boardHasPowerRail(board, rail))) {
    reasons.push(voltageRails.length === 1
      ? `${board.name} does not expose the required ${voltageRails[0]} power rail.`
      : `${board.name} does not expose any supported logic power rail (${voltageRails.join(' or ')}).`)
  }
  if (voltageRails.length === 1 && boardHasPowerRail(board, voltageRails[0]) && boardLogicVoltageDiffers(board, voltageRails[0]) && component.roles.some((role) => !role.optional && role.kind !== 'power')) {
    warnings.push(`${voltageRails[0]} part on a ${board.voltage}-logic board — a logic level shifter may be required.`)
  }
...
function boardLogicVoltageDiffers(board: BoardCatalogEntry, rail: '5V' | '3V3'): boolean {
  if (board.voltage === '5V/3.3V') return false
  return (rail === '5V' && board.voltage !== '5V') || (rail === '3V3' && board.voltage !== '3.3V')
}
...
export const CUSTOM_BOARD_VOLTAGES: Array<BoardCatalogEntry['voltage']> = ['5V', '3.3V', '5V/3.3V']
...
export function normalizeCustomComponent(raw: unknown): PinoutComponentDefinition | null {
  ...
  const definition: PinoutComponentDefinition = {
    ...
    defaultWiringNotes: wiringNotes.length > 0 ? wiringNotes : ['Custom component: confirm wiring, voltage and required libraries against the datasheet.']
  }
...
export function normalizeCustomBoard(raw: unknown): BoardCatalogEntry | null {
  ...
  const voltage = CUSTOM_BOARD_VOLTAGES.includes(input.voltage as BoardCatalogEntry['voltage']) ? (input.voltage as BoardCatalogEntry['voltage']) : '5V'
  const board: BoardCatalogEntry = {
    ...
    notes: asString(input.notes).trim() || 'Custom board defined by the user. Verify pin capabilities and voltage against the datasheet.',
...
function recommendWarnings(board: BoardCatalogEntry, req: BuildRequirement, plan: RecommendExpanderPlan): string[] {
  ...
  if (req.only3v3 && board.voltage === '5V') warnings.push('Some selected modules are 3.3V-only; add level shifting on this 5V board.')
  return warnings
}
...
function recommendPlanBoard(board: BoardCatalogEntry, req: BuildRequirement, catalog: MergedCatalog): BoardArchitecturePlan {
  // Reuse the per-component compatibility helper to reject boards on voltage,
