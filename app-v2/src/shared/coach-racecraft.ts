...
const DEFINITION_RECOMMENDATION_CLAUSE =
  /\b(?:how to|how i|how do|how should|should i|can i|could i|need to|want to|i should|i can|i could|i need|i must|we should|we need|we must|would be best|is best|when to|whether to|whether i|whether we|whether .+ (?:is|would be) (?:a )?good idea|what .+ should|which .+ should|what .+ to (?:run|use|choose|set|change|lower|raise|reduce|increase)|como faco|como posso|quando|devo|preciso|deveria|nos devemos|como puedo|cuando|debo|necesito|deberia|debemos|dois je|quand|devrais je|nous devons|serait mieux|wie ich|wie kann|wann|soll ich|muss ich|wir sollten|wir mussen|ware am besten)\b/
