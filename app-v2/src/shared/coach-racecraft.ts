...
const DEFINITION_RECOMMENDATION_CLAUSE =
  /\b(?:how to|how i|how do|how should|should i|can i|could i|need to|want to|i should|i can|i could|i need|i must|we should|we need|we must|would be best|is best|when to|whether to|whether i|whether we|whether .+ (?:is|would be) (?:a )?good idea|what .+ should|which .+ should|what .+ to (?:run|use|choose|set|change|lower|raise|reduce|increase)|como faco|como posso|quando|devo|preciso|deveria|nos devemos|qual (?:pneu|composto)|como puedo|cuando|debo|necesito|deberia|debemos|que (?:neumatico|compuesto)|dois je|quand|devrais je|nous devons|serait mieux|quel (?:pneu|compose)|wie ich|wie kann|wann|soll ich|muss ich|wir sollten|wir mussen|ware am besten|welche (?:reifen|mischung))\b/
...
export function parseDefinitionQuestion(question: string): ParsedDefinitionQuestion | null {
  ...
  const forms: ReadonlyArray<{
    ...
    },
    {
      ...
      command: false,
      patterns: [
        /^what (?:is|s) the (?:definition|meaning|explanation) of (.+)$/,
        /^what (?:is|are|s) (.+)$/,
        /^what does (.+) (?:mean|do)$/,
        /^how (?:is|are) (.+) defined$/,
