...
describe('SIM_FIELD_COVERAGE — per-provider field sets', () => {
  ...
  it('mock is FULL coverage, none is empty', () => {
    expect(SIM_FIELD_COVERAGE.mock.size).toBe(ALL_FIELDS.length + 1)
    for (const f of ALL_FIELDS) expect(SIM_FIELD_COVERAGE.mock.has(f)).toBe(true)
    expect(SIM_FIELD_COVERAGE.mock.has('liveTyrePressureKpa')).toBe(true)
    expect(SIM_FIELD_COVERAGE.none.size).toBe(0)
  })

  it('every sim set uses a known snapshot field or explicit capability', () => {
    const all = new Set<TelemetryRequirement>([
      ...ALL_FIELDS,
      'liveTyrePressureKpa'
    ])
  ...
  it('iRacing is the broadest snapshot provider while live pressure remains separate', () => {
    const ir = SIM_FIELD_COVERAGE.iracing
    for (const sim of ['acc', 'ac', 'ams2', 'lmu'] as const) {
      for (const f of SIM_FIELD_COVERAGE[sim]) {
        if (f !== 'liveTyrePressureKpa') expect(ir.has(f)).toBe(true)
      }
    }
    // Mock adds engineRunning and the explicit live-pressure capability.
...
describe('widgetSupportedSims', () => {
  ...
  it("requiring general 'tyres' returns temperature/wear sims (IR/ACC/LMU)", () => {
    expect(widgetSupportedSims(['tyres'])).toEqual(['iracing', 'acc', 'lmu'])
  })

  it('keeps live tyre pressure separate from iRacing garage cold pressure', () => {
    expect(widgetSupportedSims(['liveTyrePressureKpa'])).toEqual(['acc', 'lmu'])
    expect(widgetSupportedSims(['tireColdPressuresKpa'])).toEqual(['iracing'])
  })

  it('supports alternative requirement groups without weakening each AND-group', () => {
