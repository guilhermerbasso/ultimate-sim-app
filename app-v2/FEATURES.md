...
- **Steward Desk** — casos locais com identidades/roles estáveis e inteiramente definidos no processo main; labels do renderer nunca controlam autoria. Clips são selados por `safeStorage` no escopo do usuário Windows, persistidos atomicamente e vinculados a uma geração da sessão. Esse selo detecta corrupção e bloqueia adulteração pelo renderer, mas **não autentica origem do app nem outro processo do mesmo usuário**; por isso todo veredito autoritativo exige confirmação explícita de revisão manual. Vereditos legados sem essa confirmação são derivados como `legacy-unconfirmed`, ficam pendentes de nova adjudicação e nunca são promovidos silenciosamente. Pacotes v2 declaram esse threat model, normalizam atores/vereditos importados como claims não confiáveis e preservam anonimização por schema allowlist e gates de direitos de terceiros. A chain continua **unanchored** sem âncora externa; o app nunca aplica penalidade automática.
