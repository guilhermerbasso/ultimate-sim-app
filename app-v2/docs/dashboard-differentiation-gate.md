...
The same applies to bottom-most, fully opaque full-canvas backplates whose fill is
pixel-equivalent to `dashboard.bg`; translucent layers remain significant. Whitespace-
only transparent text layers are inert. Binding consumption is declared centrally in
`visual-pipeline/render-capabilities.ts`, while renderer defaults are materialized in
the canonical style projection.
